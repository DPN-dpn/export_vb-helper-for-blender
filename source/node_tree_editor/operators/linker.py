...
class EVHB_OT_auto_link(Operator):
    ...
    def execute(self, context):
        ...
        # 노드 분류
        asset_nodes = [n for n in node_tree.nodes if getattr(n.__class__, "bl_idname", "") == "AssetSlotNode"]
        mod_nodes = [n for n in node_tree.nodes if getattr(n.__class__, "bl_idname", "") == "ModFileNode"]
        result_node = next((n for n in node_tree.nodes if getattr(n.__class__, "bl_idname", "") == "ResultNode"), None)
        ...
        for m in mod_nodes:
            for out in m.outputs:
                mod_out_sockets.append((m, out, getattr(out.__class__, "bl_idname", ""), out.get("hash", None)))
        ...
        for a in asset_nodes:
            ...
            for inp in list(a.inputs):
                ...
                for m_node, out_sock, out_type, out_hash in mod_out_sockets:
                    ...
                    already = any(l.from_socket == out_sock and l.to_socket == inp for l in node_tree.links)
                    if already:
                        continue
                    try:
        ...
        if result_node:
            for a in assets_with_links:
                # 자산의 Result 출력 찾기
                out_sock = next((s for s in a.outputs if getattr(s.__class__, "bl_idname", "") == "ResultSocket"), None)
                if out_sock is None:
                    continue
                # 빈 입력 찾기 또는 새로 생성
                target_in = next((s for s in result_node.inputs if not s.is_linked), None)
                if target_in is None:
                    try:
                        target_in = result_node.inputs.new("ResultSocket", f"Data {len(result_node.inputs) + 1}")
                    except Exception:
                        continue
                # 중복 링크 방지
                already = any(l.from_socket == out_sock and l.to_socket == target_in for l in node_tree.links)
                if already:
                    continue
                try:
        ...
        self.report({"INFO"}, f"링크 생성: {links_created}, 결과 노드에 연결된 자산: {len(assets_with_links)}")
        return {"FINISHED"}
